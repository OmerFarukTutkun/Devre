...
constexpr int moveTypeScores[] = {MIL, MIL, 3 * MIL, 2 * MIL, 10 * MIL, 11 * MIL, 0, 0, -10 * MIL, -9 * MIL, -8 * MIL,
                                  20 * MIL, -7 * MIL, -6 * MIL, -5 * MIL, 25 * MIL};
...
constexpr int castlingSquares[N_COLORS][2][2] = {{{G1, F1}, {C1, D1}},
                                                 {{G8, F8}, {C8, D8}}};
...
inline int moveTo(uint16_t move) {
    return (move >> 4) & 63;
}

inline int moveFrom(uint16_t move) {
    return (move >> 10) & 63;
}

inline int moveType(uint16_t move) {
    return move & 15;
}

inline uint16_t createMove(int from, int to, int type) {
    return (from) << 10 | (to) << 4 | (type);
}

inline bool isPromotion(uint16_t move) {
    return (move >> 3) & 0x01;
}

inline bool isCapture(uint16_t move) {
    return (move >> 2) & 0x01;
}

inline bool isTactical(uint16_t move) {
    return (move >> 2) & 0x03;
}

inline bool isQuiet(uint16_t move) {
    return !isTactical(move);
}
