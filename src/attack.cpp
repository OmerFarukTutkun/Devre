...
bool isSquareAttacked(Board& board, int sq, int side) {
    ...
    if (side == BLACK)
    {
        return (PawnAttacks[WHITE][sq] & board.bitboards[BLACK_PAWN]) || (KnightAttacks[sq] & board.bitboards[BLACK_KNIGHT]) || (KingAttacks[sq] & board.bitboards[BLACK_KING])
            || (rookAttacks(occ, sq) & (board.bitboards[BLACK_ROOK] | board.bitboards[BLACK_QUEEN])) || (bishopAttacks(occ, sq) & (board.bitboards[BLACK_BISHOP] | board.bitboards[BLACK_QUEEN]));
    }
    else
    {
        return (PawnAttacks[BLACK][sq] & board.bitboards[WHITE_PAWN]) || (KnightAttacks[sq] & board.bitboards[WHITE_KNIGHT]) || (KingAttacks[sq] & board.bitboards[WHITE_KING])
            || (rookAttacks(occ, sq) & (board.bitboards[WHITE_ROOK] | board.bitboards[WHITE_QUEEN])) || (bishopAttacks(occ, sq) & (board.bitboards[WHITE_BISHOP] | board.bitboards[WHITE_QUEEN]));
    }
}
...
uint64_t squareAttackedBy(Board& board, int square) {
    ...
    uint64_t attackers = (knight_attacks & (board.bitboards[WHITE_KNIGHT] | board.bitboards[BLACK_KNIGHT]))
                       | (diagonal_attacks & (board.bitboards[WHITE_BISHOP] | board.bitboards[BLACK_BISHOP] | board.bitboards[WHITE_QUEEN] | board.bitboards[BLACK_QUEEN]))
                       | (horizontal_attacks & (board.bitboards[WHITE_ROOK] | board.bitboards[BLACK_ROOK] | board.bitboards[WHITE_QUEEN] | board.bitboards[BLACK_QUEEN]))
