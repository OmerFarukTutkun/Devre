...
inline uint64_t nnueHash(vecType a) {
    auto bigPositive = _mm512_set1_epi32(262144);
    auto bigNegative = _mm512_set1_epi32(-262144);

    auto positiveMask    = _mm512_cmp_epi32_mask(a, vecSetZeroEpi16(), _MM_CMPINT_NLE);
    auto bigPositiveMask = _mm512_cmp_epi32_mask(a, bigPositive, _MM_CMPINT_NLE);
    auto bigNegativeMask = _mm512_cmp_epi32_mask(bigNegative, a, _MM_CMPINT_NLE);

    bigPositiveMask &= positiveMask;
    bigNegativeMask &= ~positiveMask;

    uint32_t mask = (positiveMask & 255) | ((positiveMask >> 8) << 16);
    uint32_t m    = (bigPositiveMask | bigNegativeMask);
    m = (m & 255) | ((m >> 8) << 16);
    return mask | (m << 8);
}
