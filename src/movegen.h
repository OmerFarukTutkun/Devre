...
uint64_t CheckMask(const Board &board, int sq, uint64_t occAll, int &numCheck) {
    ...
    uint64_t rook_mask =
            (board.bitboards[pieceIndex(~c, ROOK)] | board.bitboards[pieceIndex(~c, QUEEN)]) & rookAttacks(occAll, sq);
...
template<Color c>
uint64_t pinMaskRooks(const Board &board, int sq, uint64_t occ_us,
                      uint64_t occ_enemy) {
    uint64_t rookMask =
    ...
    while (rookMask) {
        ...
        if (popcount64(possible_pin & occ_us) == 1) pin_hv |= possible_pin;
    }
    return pin_hv;
}

template<Color c>
uint64_t pinMaskBishops(const Board &board, int sq, uint64_t occ_us,
                        uint64_t occ_enemy) {
    uint64_t bishopMask =
            (board.bitboards[pieceIndex(~c, BISHOP)] | board.bitboards[pieceIndex(~c, QUEEN)]) &
    ...
    while (bishopMask) {
        ...
        const uint64_t possible_pin = (SQUARES_BETWEEN[sq][index] | (ONE << index));
        if (popcount64(possible_pin & occ_us) == 1) pin_d |= possible_pin;
    }
    return pin_d;
}
...
void generatePieceMoves(uint64_t pieces, uint64_t movable, uint64_t occAll, uint64_t occEnemy, uint64_t pinMask,
                        MoveList &movelist) {
...
template<Color c>
void generateCastlingMoves(const Board &board, int kingSq, uint64_t occ, uint64_t seen, MoveList &moveList) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (canCastle) {
            ...
            //supports frc
            uint64_t path =
                    SQUARES_BETWEEN[kingSq][kingTo] | SQUARES_BETWEEN[rook][rookTo] | (ONE << kingTo) | (ONE << rookTo);
...
void generateLegalPawnMoves(const Board &board, int kingSq, uint64_t occAll, uint64_t occEnemy,
                            uint64_t check_mask, uint64_t pin_hv, uint64_t pin_d, MoveList &movelist) {
    ...
    if (type == ALL_MOVES || type == TACTICAL_MOVES) {
        if (board.enPassant != NO_SQ) {
            ...
            uint64_t enemy_queen_rook = (board.bitboards[pieceIndex(~c, ROOK)] |
                                         board.bitboards[pieceIndex(~c, QUEEN)]);
            uint64_t enemy_queen_bishop = (board.bitboards[pieceIndex(~c, BISHOP)] |
                                           board.bitboards[pieceIndex(~c, QUEEN)]);
            while (epBB) {
