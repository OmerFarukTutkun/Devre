...
void updateQuietHistories(ThreadData &thread, Stack *ss, int depth, uint16_t bestMove) {
    ...
    for (int i = 0; i < ss->played; i++) {
        ...
        if (isQuiet(move)) {
            ...
            int16_t *current = &thread.history[checkBit(ss->threat, from)][checkBit(ss->threat,
                                                                                    to)][board->sideToMove][from][to];
...
int getCaptureHistory(ThreadData &thread, Stack *ss, uint16_t move) {
    ...
    int from = moveFrom(move);
    int to = moveTo(move);
    return thread.captureHist[board->sideToMove][pieceType(board->pieceBoard[from])][to][pieceType(
            board->pieceBoard[to])];
}
...
void updateCorrHistScore(ThreadData &thread, Stack *ss, const int depth, const int diff) {
    ...
    int &nnueCorrHistEntry = thread.nnueCorrHist[board->sideToMove][0][board->nnueKey & 65535];
    int &nnueCorrHistEntry2 = thread.nnueCorrHist[board->sideToMove][1][(board->nnueKey >> 16) & 65535];
    ...
    if (isMoveOk) {
        ...
        auto &threatLastMoveCorrHistEntry = thread.threatLastMoveCorrHist[checkBit((ss - 1)->threat, from)][checkBit(
...
int adjustEvalWithCorrHist(ThreadData &thread, Stack *ss, const int rawEval) {
    ...
    int nnueCorrHistScore = thread.nnueCorrHist[board->sideToMove][0][board->nnueKey & 65535];
    nnueCorrHistScore += thread.nnueCorrHist[board->sideToMove][1][(board->nnueKey >> 16) & 65535];
