...
int Search::alphaBeta(int alpha, int beta, int depth, const bool cutNode, ThreadData& thread, Stack* ss) {
    ...
    while ((move = moveList.pickMove(thread, ss)) != NO_MOVE)
    {
        ...
        int extension = 0;
        if (ss->ply < thread.searchDepth && !rootNode && depth >= 8 && move == ttMove && ss->excludedMove == NO_MOVE && (ttBound & TT_LOWERBOUND)
            && ttDepth >= depth - 3)
        {
    ...
    if (ss->excludedMove == NO_MOVE)
    {
        ...
        if (!inCheck && (!bestMove || !isTactical(bestMove)) && !(bound == TT_LOWERBOUND && bestScore <= ss->staticEval)
            && !(bound == TT_UPPERBOUND && bestScore >= ss->staticEval))
        {

            updateCorrHistScore(thread, ss, depth, bestScore - ss->staticEval);
            updateEvalHistScore(thread,ss, depth, bestScore - getEvalHist(thread,ss));
        }
...
SearchResult Search::start(Board* board, TimeManager* tm, int ThreadID) {
    ...
    for (int i = 1; i <= timeManager->depthLimit; i++)
    {
        ...
        if (ThreadID == 0)
        {
            ...
            std::cout << " nps " << nps << " nodes " << nodes << " time " << elapsed << " hashfull " << TT::Instance()->getHashfull() << " tbhits "
                      << totalTbHits() << " pv " << getPV(ss + 6, threads.at(ThreadID)->board) << std::endl;
