...
uint32_t probeTB(Board& pos) {
    ...
    return tb_probe_wdl(white, black, kings, queens, rooks, bishops, knights, pawns, pos.halfMove, pos.castlings,
                        pos.enPassant == NO_SQ ? 0 : pos.enPassant, pos.sideToMove == WHITE);
}
...
int Search::alphaBeta(int alpha, int beta, int depth, const bool cutNode, ThreadData& thread, Stack* ss) {
    ...
    if (!PVNode && ss->excludedMove == NO_MOVE && depth <= 8 && (!ttMove || ttCapture) && eval > beta + depth * 107
        && !rootNode)
    {
        return eval;
    }
    ...
    //Razoring
    if (!PVNode && ss->excludedMove == NO_MOVE && depth <= 4 && eval + 408 * depth < alpha)
    {
    ...
    while ((move = moveList.pickMove(thread, ss)) != NO_MOVE)
    {
        ...
        int extension = 0;
        if (ss->ply < thread.searchDepth && !rootNode && depth >= 8 && move == ttMove && ss->excludedMove == NO_MOVE
            && (ttBound & TT_LOWERBOUND) && ttDepth >= depth - 3)
        {
    ...
    if (ss->excludedMove == NO_MOVE)
    {
        ...
        if ((!bestMove || !isTactical(bestMove)) && !(bound == TT_LOWERBOUND && bestScore <= ss->staticEval)
...
SearchResult Search::start(Board* board, TimeManager* tm, int ThreadID) {
    ...
    for (int i = 1; i <= timeManager->depthLimit; i++)
    {
        ...
        if (ThreadID == 0)
        {
            ...
            std::cout << " nps " << nps << " nodes " << nodes << " time " << elapsed << " hashfull " << TT::Instance()->getHashfull()
                      << " tbhits " << totalTbHits() << " pv " << getPV(ss + 6, threads.at(ThreadID)->board) << std::endl;
